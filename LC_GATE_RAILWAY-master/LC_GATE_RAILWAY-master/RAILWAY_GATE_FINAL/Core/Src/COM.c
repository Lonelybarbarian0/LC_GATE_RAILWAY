...
volatile uint8_t tx_buff[13] = {0xFF}; /* Transmitted Message */
volatile uint8_t temp_tx_buff[13] = {0xFF}; /* Transmitting Message */
extern uint8_t flag;
...
void Transmit_Msg()
{
	...
	tx_buff[11] = temp_tx_buff[11]; /* Lock OFF Sensor Health */
	tx_buff[12] = temp_tx_buff[12]; /* Lock ON Sensor Health */

	CDC_Transmit_FS((uint8_t*)tx_buff,13);
}
